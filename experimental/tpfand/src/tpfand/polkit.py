...
class Polkit(object):
    ...
    def check_status(self, action_id, sender, link):
        ...
        try:
            dbus_link = link.get_object("org.freedesktop.DBus","/org/freedesktop/DBus/Bus", False)
            dbus_info = dbus.Interface(dbus_link,"org.freedesktop.DBus")
            pid = dbus_info.GetConnectionUnixProcessID(sender)
            print "Checking now"
            (pk_allowed,pk_extra,pk_info) = self.pk_authority.CheckAuthorization(("unix-process", {"pid": dbus.UInt32(pid, variant_level=1), "start-time":dbus.UInt64(0,variant_level=1)}),action_id, {"": ""}, dbus.UInt32(0),"",timeout=600)
        except Exception, ex:
    ...
    def authorize(self, action_id, sender, link):
        ...
        try:
            ...
            dbus_info = dbus.Interface(dbus_link,"org.freedesktop.DBus")
            pid = dbus_info.GetConnectionUnixProcessID(sender)
            (pk_allowed,pk_extra,pk_info) = self.pk_authority.CheckAuthorization(("unix-process", {"pid": dbus.UInt32(pid, variant_level=1), "start-time":dbus.UInt64(0,variant_level=1)}),action_id, {"": ""}, dbus.UInt32(1),"",timeout=600)
        except Exception, ex:
...
def main():
    ...
    try:
        ...
        policy_kit = system_bus.get_object("org.freedesktop.PolicyKit1","/org/freedesktop/PolicyKit1/Authority")
        pk_authority = dbus.Interface(policy_kit, "org.freedesktop.PolicyKit1.Authority")
